...
app.get('/ba_reference', async (req, res) => {
  const clientId = process.env.CLIENT_ID;
  try {
    res.render('ba_reference', { clientId });
  } catch (err) {
    handleError(res, err);
  }
});
...
app.post('/api/ba/create-billing-token', async (req, res) => {
  try {
...
app.post('/api/ba/capture-order', async (req, res) => {
  try {
